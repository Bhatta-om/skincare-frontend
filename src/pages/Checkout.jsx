...
export default function Checkout() {
  ...
  const [formData, setFormData] = useState({ full_name: '', email: '', phone: '', address_line1: '', city: '', payment_method: 'esewa' })

  useEffect(() => {
    if (user) setFormData(f => ({ ...f, full_name: `${user.first_name||''} ${user.last_name||''}`.trim(), email: user.email||'', phone: user.phone||'' }))
  }, [user])

  useEffect(() => {
    if (buyNowData) { setLoading(false); return }
    api.get('/orders/cart/').then(res => setCart(res.data?.cart||res.data)).catch(() => setCart(null)).finally(() => setLoading(false))
  }, [])
  ...
  const submitEsewaForm = (esewaFormData, esewaUrl) => {
    ...
    form.method = 'POST'; form.action = esewaUrl
    Object.keys(esewaFormData).forEach(key => { const input = document.createElement('input'); input.type='hidden'; input.name=key; input.value=esewaFormData[key]; form.appendChild(input) })
    document.body.appendChild(form); form.submit()
  }

  const handleSubmit = async (e) => {
    e.preventDefault(); setPlacing(true)
    const toastId = toast.loading(formData.payment_method === 'esewa' ? 'Redirecting to eSewa...' : 'Placing your order...')
    try {
      let orderId
      if (buyNowData) {
        const res = await api.post('/orders/buy-now/', { product_id: buyNowData.product_id, quantity: buyNowData.quantity, full_name: formData.full_name, phone: formData.phone, email: formData.email, address_line1: formData.address_line1, city: formData.city, payment_method: formData.payment_method })
        orderId = res.data.order.id
      } else {
        const res = await api.post('/orders/create/', formData)
        orderId = res.data.order.id; await fetchCartCount()
      }
      ...
      } else { toast.success('Order placed successfully', { id: toastId }); navigate('/orders') }
    } catch (err) {
      const msg = err.response?.data?.error || err.response?.data?.detail || 'Failed to place order.'
      toast.error(msg, { id: toastId }); setPlacing(false)
    }
  }
  ...
  const items    = buyNowData ? [{ id:1, product:buyNowData.product, quantity:buyNowData.quantity, total_price:(parseFloat(buyNowData.product?.discounted_price||0)*buyNowData.quantity).toFixed(2) }] : (cart?.items||[])
  const subtotal = buyNowData ? parseFloat(buyNowData.product?.discounted_price||0)*buyNowData.quantity : parseFloat(cart?.subtotal||cart?.total_amount||0)
        ...
        <div style={{ background: '#F4EDE4', borderBottom: '1px solid #E6DDD3', padding: 'clamp(32px,5vw,48px) 0 40px' }}>
          <div className="container-luxury">
            <div className="breadcrumb" style={{ marginBottom: '20px' }}>
              <a href="/cart" style={{ color: '#AA9688', textDecoration: 'none' }}>Cart</a>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              {buyNowData && <span className="badge badge-accent" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Zap size={10} strokeWidth={1.5} /> Quick Buy</span>}
            </div>
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(26px,3vw,36px)', color: '#16100C', fontWeight: 400, marginTop: '8px' }}>Checkout</h1>
          </div>
        </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <SectionCard step="2" title="Payment Method">
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <PaymentOption value="esewa" selected={formData.payment_method==='esewa'} onChange={handleChange} label="eSewa" desc="Fast and secure digital wallet payment" icon={<span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '10px', fontWeight: 500, color: '#16a34a' }}>eSewa</span>} />
                    <PaymentOption value="cod"   selected={formData.payment_method==='cod'}   onChange={handleChange} label="Cash on Delivery" desc="Pay in cash when your order arrives" icon={<CreditCard size={16} strokeWidth={1.5} style={{ color: '#B8895A' }} />} />
                  </div>
                    ...
                    <div style={{ marginTop: '14px', background: '#EEF6F1', border: '1px solid #C4DAC8', padding: '14px 16px', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '12px', color: '#7AAA87', fontWeight: 300 }}>
                          ID: <span style={{ fontFamily: 'monospace', fontWeight: 500, color: '#4A7A57' }}>9806800001</span> · Password: <span style={{ fontFamily: 'monospace', fontWeight: 500, color: '#4A7A57' }}>Nepal@123</span>
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <div className="checkout-summary-sticky">
                <div style={{ background: '#FFFFFF', border: '1px solid #E6DDD3', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: 'clamp(14px,2vw,20px) clamp(16px,3vw,24px)', borderBottom: '1px solid #EEE7DF' }}>
                    ...
                    {buyNowData && <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '11px', color: '#AA9688', marginTop: '3px', fontWeight: 300 }}>Quick Buy — cart unchanged</p>}
                  </div>
                    ...
                    {items.map(item => (
                      <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '48px 1fr auto', gap: '12px', alignItems: 'center' }}>
                        <div style={{ width: '48px', height: '48px', background: '#F4EDE4', border: '1px solid #E6DDD3', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {item.product?.image ? <img src={getProductImageUrl(item.product?.image)} alt={item.product?.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Package size={18} strokeWidth={1} style={{ color: '#D4C4B0' }} />}
                        </div>
                  ...
                  <div style={{ padding: 'clamp(16px,3vw,20px) clamp(16px,3vw,24px)' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '14px' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {delivery===0 ? <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '12.5px', color: '#4A7A57', fontWeight: 400 }}>Free</span> : <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '12.5px', color: '#16100C', fontWeight: 400 }}>Rs. {delivery}</span>}
                      </div>
                    </div>
                    ...
                    <button type="submit" disabled={placing}
                      style={{ width: '100%', background: formData.payment_method==='esewa'?'#16a34a':'#16100C', color: '#FFFFFF', border: 'none', padding: '15px', fontFamily: "'DM Sans',sans-serif", fontSize: '11.5px', fontWeight: 400, textTransform: 'uppercase', letterSpacing: '0.14em', cursor: placing?'not-allowed':'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', opacity: placing?0.7:1, transition: 'background 0.25s' }}
                      onMouseEnter={e => { if(!placing) e.currentTarget.style.background = formData.payment_method==='esewa'?'#15803d':'#3A2820' }}
                      onMouseLeave={e => { if(!placing) e.currentTarget.style.background = formData.payment_method==='esewa'?'#16a34a':'#16100C' }}
                    >
                      <Lock size={13} strokeWidth={1.5} />
                      {placing ? 'Processing...' : formData.payment_method==='esewa' ? 'Pay with eSewa' : 'Place Order (COD)'}
                    </button>
                    <div className="checkout-trust-badges">
                      {[{ icon:<Lock size={13} strokeWidth={1.5} />, label:'Secure' },{ icon:<ShieldCheck size={13} strokeWidth={1.5} />, label:'eSewa' },{ icon:<CreditCard size={13} strokeWidth={1.5} />, label:'COD' }].map(({icon,label}) => (
                    ...
                    <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '11px', color: '#AA9688', textAlign: 'center', marginTop: '10px', fontWeight: 300 }}>100% secure &amp; encrypted checkout</p>
                  </div>
                </div>
              </div>
            </div>
